...
export const Pane = React.forwardRef<HTMLDivElement, PaneProps>(
  (
    ...
    // Determine responsive width classes based on role if no custom width provided
    let widthClass = "";
    if (!width) {
      if (role === "list")
        widthClass =
    ...
    // Default: Show scrollbar for main pane, hide for others (legacy behavior but cleaner)
    // Actually, for better UX, we might want to default to visible if not specified,
    // but to avoid breaking changes, let's keep 'no-scrollbar' unless showScrollbar is true.
    // WAIT: The user wants to FIX layout shifting.
    // If I default to visible (showScrollbar=true), layout shifts will happen unless I use gutter stable.
    // If I use gutter stable, I need visible scrollbars.
    // So for main pane, I will default to visible with gutter stable.
