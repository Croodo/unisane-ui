...
const drawerItemVariants = cva(
  "flex items-center gap-3 w-full min-h-14 py-4 px-5 rounded-xl text-label-large font-medium transition-all duration-short ease-emphasized cursor-pointer select-none group focus-visible:outline-2 focus-visible:outline-primary relative overflow-hidden shrink-0 mx-4",
  {
    variants: {
      active: {
...
export const NavigationDrawerItem: React.FC<NavigationDrawerItemProps> = ({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span className="text-label-small font-black relative z-10 bg-on-secondary-container text-secondary-container px-1_5 py-0_5 rounded-xs">
          {badge}
        </span>
      )}
    </button>
...
export const NavigationDrawerHeadline: React.FC<{
  ...
  <div className="px-6 py-4 text-label-small font-black uppercase tracking-[0.2em] text-on-surface-variant/60 truncate">
    {children}
  </div>
);
