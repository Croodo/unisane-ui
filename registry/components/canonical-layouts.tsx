  ...
  return (
    <div
      className={cn(
        ...
        !isRoot && "rounded-xs border border-outline-variant/30",
        // Expanded+ Grid: Supporting width when open, Rail width when closed
        isOpen
    ...
    >
      ...
      <aside
        className={cn(
          ...
          // Expanded+: Static positioning, remove shadow
      ...
      >
        {isOpen ? (
          ...
          <div className="hidden expanded:flex flex-col items-center py-6 h-full gap-4 overflow-y-auto no-scrollbar">
            ...
            <div className="rotate-90 whitespace-nowrap text-label-small font-black uppercase text-on-surface-variant/40 tracking-[0.4em] origin-center mt-12 mb-6 shrink-0">
              {title}
            </div>
          </div>
        )}
      ...
      <div
        className={cn(
          "expanded:hidden absolute inset-0 bg-scrim/30 backdrop-blur-[calc(var(--unit)/4)] z-10 transition-opacity duration-emphasized ease-emphasized",
