    ...
    >
        ...
        <div className="relative w-full h-full flex items-center px-4">
          ...
          {label && (
            <label
              ...
              id={labelId}
              className={cn(
                "absolute pointer-events-none truncate max-w-[calc(100%-calc(var(--uni-sys-u)*12))] transition-all duration-snappy ease-emphasized origin-left left-4",
                !isFloating &&
      ...
      {isOpen && !disabled && (
        <div
          ...
          role="listbox"
          aria-labelledby={label ? labelId : undefined}
          className="absolute top-[calc(100%+var(--uni-sys-u))] left-0 w-full bg-surface border border-outline-variant rounded-sm shadow-3 py-1 max-h-[calc(var(--uni-sys-u)*70)] overflow-y-auto z-[100] animate-in fade-in zoom-in-95 duration-snappy"
        >
          {options.length > 0 ? (
            options.map((option, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "px-4 h-11 flex items-center text-body-small font-medium cursor-pointer transition-colors",
