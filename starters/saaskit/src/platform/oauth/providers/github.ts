...
async function fetchGithubProfile(accessToken: string): Promise<ProviderProfile> {
  // Expect a user access token obtained via OAuth code flow on the client
  const headers = { Authorization: `Bearer ${accessToken}`, 'user-agent': 'saaskit' } as const;
  ...
  const displayName = (user.name as string | undefined) ?? (user.login as string | undefined) ?? null;
  const imageUrl = (user.avatar_url as string | undefined) ?? null;
  return { provider: 'github', subject: id, email, displayName, imageUrl };
}
