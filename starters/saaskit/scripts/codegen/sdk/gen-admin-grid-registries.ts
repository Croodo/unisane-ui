/*
  AUTO-GENERATOR: emits UI-safe grid field registries + list param hooks based on gateway registries.
  Run with: pnpm sdk:gen (chains after devtools sdk:gen)
*/
import fs from "node:fs";
import path from "node:path";

// Import from @unisane/gateway package (monorepo structure)
import type { FieldDef } from "@unisane/gateway";
import {
  adminListConfigs,
...
function emitFields(name: string, reg: Record<string, FieldDef>) {
  const out: Record<string, UiFieldDef> = {};
  for (const [k, v] of Object.entries(reg)) {
    const enumValues = (v as { enumValues?: readonly string[] | string[] })
      .enumValues;
    out[k] = {
      type: v.type as UiFieldDef["type"],
      ...(Array.isArray(v.ops) ? { ops: v.ops } : {}),
      ...(Array.isArray(enumValues) ? { enumValues } : {}),
    } as UiFieldDef;
  }
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n`;
  ...
  const dir = path.join(process.cwd(), "src/sdk/registries/generated");
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    path.join(dir, `${name}.fields.gen.ts`),
    header + body,
    "utf8"
  );
}
...
for (const cfg of adminListConfigs) {
  emitFields(cfg.id, cfg.fieldsRegistry as Record<string, FieldDef>);
}

console.log(
  `Generated UI grid registries: ${adminListConfigs.map((c) => c.id).join(", ")}`
);

function emitListParamsHooks(configs: AdminListConfig[]) {
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n`;
  const imports = [
...
`;
  ...
  const listObjectParts: string[] = [];
  listObjectParts.push("export const listParamsHooks = {");
  ...
  const dir = path.join(process.cwd(), "src/sdk/hooks/generated");
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "admin-list-params.ts"), content, "utf8");
}

emitListParamsHooks(adminListConfigs);

// Emit the generic list params helper (so the SDK hooks remain fully generated).
function emitBaseListHelper() {
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n"use client";\n`;
  const body = `
...
export const useAdminListParams = ({
  ...
  // useState only uses initial value on mount, so we need useEffect to sync on URL changes
  useEffect(() => {
...
`;
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    path.join(dir, "useAdminListParams.ts"),
    header + body,
    "utf8"
  );
}
...
// Emit admin registries with filter mappers + facets driven from the manifest.
function emitAdminRegistries(configs: AdminListConfig[]) {
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n`;
...
`;
    const dir = path.join(process.cwd(), "src/sdk/registries");
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, filename), content, "utf8");
  }
}

emitAdminRegistries(adminListConfigs.filter((c) => c.id.startsWith("admin.")));

// Emit hooks/index.ts to wire generated hooks + list params helpers.
function emitHooksIndex() {
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n`;
...
`;
  const dir = path.join(process.cwd(), "src/sdk/hooks");
  fs.mkdirSync(dir, { recursive: true });
...
// Emit tenant grids (simpler pattern than admin grids)
function emitTenantRegistries() {
  const header = `/* AUTO-GENERATED by scripts/codegen/sdk/gen-admin-grid-registries.ts — DO NOT EDIT */\n`;

  // Tenant webhooks grid
  const webhooksContent = `${header}import { WEBHOOK_DIRECTION, WEBHOOK_EVENT_STATUS } from '@unisane/kernel/client';
...
`;
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    path.join(dir, "tenant.webhooks.grid.ts"),
    webhooksContent,
    "utf8"
  );
  console.log("Generated tenant registries: tenant.webhooks");
}

emitTenantRegistries();

console.log("Admin grid registries generation complete!");
