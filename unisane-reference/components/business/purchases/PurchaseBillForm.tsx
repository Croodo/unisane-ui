...
export const PurchaseBillForm = ({ onSave, onCancel }: any) => {
  ...
  const handleSelectItem = (selectedItem: any) => {
    ...
    newItems[activeRowIdx] = {
      ...
      rate: selectedItem.purchasePrice || selectedItem.price,
      tax: parseInt(selectedItem.tax),
      amount: (newItems[activeRowIdx].qty * (selectedItem.purchasePrice || selectedItem.price))
    };
    setItems(newItems);
  };
  ...
  const addItemRow = () => setItems([...items, { itemId: '', name: '', qty: 1, rate: 0, tax: 18, amount: 0 }]);
    ...
    <div className="flex flex-col bg-white">
      <div className="p-8 md:p-10 flex flex-col gap-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
             <span className="text-[10px] font-black text-stone-400 uppercase tracking-widest absolute top-2 left-4">Supplier</span>
             <div className="flex items-center justify-between pt-3">
                <span className={cn("font-bold truncate text-sm uppercase", supplier ? "text-stone-900" : "text-stone-300")}>
                    {supplier?.name || "Select Supplier..."}
                </span>
                <Icons.Parties size={16} className="text-stone-300 group-hover:text-primary" />
             </div>
          </div>
          <TextField label="SUPPLIER BILL NO" value={billNo} onChange={e => setBillNo(e.target.value)} labelClassName="bg-white" />
          <TextField label="BILL DATE" type="date" value={date} onChange={e => setDate(e.target.value)} labelClassName="bg-white" />
        </div>
        ...
        <div className="flex flex-col gap-4">
           ...
           {items.map((item, idx) => (
             <div key={idx} className="grid grid-cols-[2fr_1fr_1fr_1fr_100px_48px] gap-3 items-start animate-in slide-in-from-top-2 duration-300">
                ...
                <TextField type="number" label="Qty" value={item.qty} onChange={e => updateItem(idx, 'qty', Number(e.target.value))} labelClassName="bg-white" />
                {/* Fixed: Replaced non-existent updateLineItemRate with updateItem using 'rate' field */}
                <TextField type="number" label="Rate" value={item.rate} onChange={e => updateItem(idx, 'rate', Number(e.target.value))} labelClassName="bg-white" />
                <TextField type="number" label="Tax" value={item.tax} onChange={e => updateItem(idx, 'tax', Number(e.target.value))} labelClassName="bg-white" />
                ...
                <IconButton onClick={() => setItems(items.filter((_, i) => i !== idx))} className="mt-2 text-stone-300 hover:text-error">
                    <Icons.Delete size={20} />
                </IconButton>
             </div>
           ))}
           <IconButton onClick={addItemRow} className="rounded-xs bg-stone-100 text-stone-900 hover:bg-stone-900 hover:text-white transition-all">
             <Icons.Add />
           </IconButton>
        </div>
      ...
      <ItemPicker open={isPickerOpen} onClose={() => setIsPickerOpen(false)} onSelect={handleSelectItem} />
      <PartyPicker open={isPartyPickerOpen} onClose={() => setIsPartyPickerOpen(false)} onSelect={setSupplier} type="suppliers" />
    </div>
  );
};