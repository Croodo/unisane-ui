...
export async function readAdminTenant(args: ReadAdminTenantArgs): Promise<{
  ...
  const [mMap, apiMap, ovMap, invMap, latestSubMap, actMap, whMap, crMap] =
    await Promise.all([
      providers.getScopeMembershipCounts?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeApiKeyCounts?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeOverrideCounts?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeOpenInvoiceCounts?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeLatestSubscriptions?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeLastActivity?.([args.tenantId]) ?? Promise.resolve(new Map()),
      providers.getScopeFailureCounts?.([args.tenantId], since24h) ?? Promise.resolve(new Map()),
      providers.getScopeCreditBalances?.([args.tenantId]) ?? Promise.resolve(new Map()),
    ]);
