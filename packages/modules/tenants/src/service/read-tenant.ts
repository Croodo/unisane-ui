 ...
 *
 * Note: This function accepts explicit scopeId for cross-module use cases
 ...
 * where the caller may not have tenant context (e.g., admin operations).
 */
export async function readTenant(scopeId: string): Promise<TenantRow | null> {
  const key = tenantKeys.byId(scopeId);
  const cached = await cacheGet<TenantRow>(key);
  if (cached) return cached;
  const row = await TenantsRepo.findById(scopeId);
