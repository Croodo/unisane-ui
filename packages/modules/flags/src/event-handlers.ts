...
async function handleSubscriptionUpdated(payload: {
  ...
  try {
    ...
    await cacheDelete(`${FLAGS_CACHE_PREFIX}tenant:${tenantId}`);
...
async function handleTenantDeleted(payload: {
  ...
  log.info('handling tenant deletion for flags cleanup', { tenantId });
  ...
  try {
    ...
    log.info('flags cleanup completed for deleted tenant', { tenantId });
  } catch (error) {
...
async function handleSettingsUpdated(payload: {
  ...
  log.info('handling settings update for flags', { tenantId, key });

  try {
    // Invalidate cached flags since settings changed
    await cacheDelete(`${FLAGS_CACHE_PREFIX}tenant:${tenantId}`);

    log.debug('flags cache invalidated for settings change', { tenantId, key });
  } catch (error) {
